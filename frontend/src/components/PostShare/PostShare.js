...
const PostShare = (props) => {
  ...
  //image selecting and displaying
  const onImageChange = (event) => {
    if (event.target.files && event.target.files[0]) { // if file is present and it is in the index of 0
      ...
      <div>
        ...
        <div className="postOptions">
          <div style={{color: "var(--photo)"}}
               onClick={() => imageRef.current.click()}>{/* when click the icon the image ref clicked */}
            <UilScenery/>
          </div>
          <div
      ...
      <div>
        {image && (
          <div className="previewImage">
            ...
            <img src={URL.createObjectURL(image)} alt=""/> {/* to display image after choosing  */}
          </div>
        )}
      </div>
    </div>
